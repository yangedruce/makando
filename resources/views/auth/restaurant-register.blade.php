...
<x-layouts.auth>
    ...
    <form method="POST" action="{{ route('restaurant.register') }}" class="mt-8">
        @csrf
        <div class="space-y-4">
            <div class="space-y-2">
            ...
            <div class="space-y-2">
                <x-label for="restaurant_name">{{ __('Restaurant Name') }}</x-label>
                <x-input id="restaurant_name" name="restaurant_name" value="{{ old('restaurant_name') }}"
                    placeholder="{{ __('Enter restaurant name') }}" required />
