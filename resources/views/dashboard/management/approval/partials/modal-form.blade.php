<x-modal name="confirm-approval">
    <div class="space-y-8">
        <div class="space-y-1">
            ...
            <x-text x-text="`{{ __('You are about to update the status of restaurant ${name}.') }}`"></x-text>
        </div>
        <div class="flex justify-end gap-2">
            <x-button style="secondary" @click="$dispatch('close')">{{ __('Cancel') }}</x-button>
            <form :action="`/dashboard/management/restaurant-approval/update/${id}`" method="POST" id="update-status-form">
                @csrf
                @method('PATCH')
