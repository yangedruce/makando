...
<x-layouts.dashboard>
    ...
    <x-card>
        <div class="space-y-8">
            ...
            <form method="post" action="{{ route('dashboard.management.customer.store') }}" x-data="{
                validateForm() {
                    $el.reportValidity();
                    if ($el.checkValidity()) {
                        $el.submit();
                ...
                <div class="max-w-xl space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <x-input type="email" id="email" name="email" value="{{ old('email', $customer->user->email ?? '') }}" placeholder="{{ __('Enter email address') }}" />
                        <x-input-error :messages="$errors->get('email')" />
                    </div>
                    <div class="space-y-2">
                        <x-label for="phone_no">{{ __('Phone Number') }}</x-label>
                        <x-input type="text" id="phone_no" name="phone_no" value="{{ old('phone_no', $customer->phone_no ?? '') }}" placeholder="{{ __('Enter phone number') }}" />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-input type="text" id="address" name="address" value="{{ old('address') }}"
                            placeholder="{{ __('Enter customer address') }}" />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-input type="number" id="total_points" name="total_points" min="0" value="{{ old('total_points', $customer->total_points ?? 0) }}" placeholder="Enter total points" />
